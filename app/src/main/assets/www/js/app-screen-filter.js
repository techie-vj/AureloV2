...
window.ScreenFilter = (function () {
 ...
 // FIX 1 + 2: excluded apps — camera shown as single chip
  ...
  /* ── Active-strip helpers ──────────────────────────────────────── */

  /**
   * Returns a human-readable end-time string for the active filter.
   * Exported as ScreenFilter.getEndStr() for use in home-habits strip.
   */
  function _getFilterEndStr(cfg) {
    var pad = function (n) { return n < 10 ? '0' + n : '' + n; };
    if (!cfg.schedule || cfg.schedule === 'none') return 'On manually';
    if (cfg.schedule === 'sun') {
      if (cfg.sunriseHour == null) return 'Until sunrise';
      var h = cfg.sunriseHour, m = cfg.sunriseMin || 0;
      var h12 = h % 12 === 0 ? 12 : h % 12;
      return 'Until sunrise \u00b7 ' + h12 + ':' + pad(m) + (h >= 12 ? ' PM' : ' AM');
    }
    // custom schedule
    var eh = cfg.schedEndHour != null ? cfg.schedEndHour : 7;
    var em = cfg.schedEndMin  != null ? cfg.schedEndMin  : 0;
    var h12 = eh % 12 === 0 ? 12 : eh % 12;
