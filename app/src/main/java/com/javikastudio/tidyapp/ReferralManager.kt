...
object ReferralManager {
    ...
    fun redeemReferralCode(prefs: SharedPreferences, code: String): JSONObject {
        ...
        // BUG-REF-7 FIX: derive the expected verifier from THIS device's own
        // referral code before examining the incoming code. Any code that was not
        // generated by a referred device that installed via THIS referrer's link
        // will not carry the correct verifier and is rejected immediately —
        // closing the "any 8-char string gives 3 days" exploit.
        val myCode = getMyReferralCode(prefs)

        return when {
            cleaned.length == 8 && cleaned.all { it.isLetterOrDigit() } -> {
                // Install confirmation code format (BUG-REF-7):
                //   sha256(referrerCode).take(2)  ← verifier prefix
                // + sha256(referrerCode+installId+"install").take(6)
            ...
            cleaned.length == 9 && cleaned[0] in listOf('M', 'A', 'L') ->
                cleaned.drop(1).let { rest ->
                    if (rest.all { it.isLetterOrDigit() }) {
                        ...
                        // Conversion confirmation code format (BUG-REF-7):
                        //   planPrefix(1) + sha256(referrerCode+plan).take(2) + sha256(...).take(6)
